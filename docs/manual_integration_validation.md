...
| Generate Draft atomic overwrite | Generate a long Draft, then a shorter Draft, with approved write-failure injection between checks. Put sentinel values outside `I1` and the signed threshold/lookup-control rows. | Shorter generation clears stale `A:G` and Notes-column `H`, removes a threshold only when its exact Japanese label signs the expected row, migrates either legacy or shifted lookup labels, clears only their old bot-owned formatting, rebuilds controls at the new rows, and preserves unrelated right-side values and formatting. The injected failed value/formula/background/border/frozen-column batch leaves the previous Draft intact. |  |  |
