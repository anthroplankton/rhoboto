...
| Settings embed with one Team source | Configure exactly one Team Register in the same guild, then run `/shift_register settings`. | Embed shows worksheet titles/IDs, Sheet link, Team Summary Source channel and live Summary worksheet link/ID, final schedule anchor cell, Shift Timeline, and Recruitment Time Range. |  |  |
| Missing Team source | Remove all Team Register configuration from the development guild, then reopen Shift settings. | Team Summary Source says no configured Team Register exists. Shift registration still updates availability. |  |  |
| Ambiguous Team source | Configure Team Register in two channels in the same guild, then reopen Shift settings. | Team Summary Source reports multiple configured Team Registers and that source selection is not yet supported. Shift registration still updates availability and stale `C` anchors are cleared. |  |  |
| Invalid Team source | With one Team Register, remove a configured Team/Summary worksheet or alter the required Summary header, then reopen Shift settings and submit a Shift. | Settings reports an invalid Team Summary source; Shift availability still updates, stale `C` anchors are cleared, and no guessed Team columns are used. |  |  |
