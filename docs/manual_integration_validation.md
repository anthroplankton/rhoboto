...
| Generate Draft LLM prompt snapshot | Generate with recruitment gaps, a fixed Runner who also submitted Shift Entry and shares a display name, assigned and unassigned participants, duplicate/reserved canonical names, complete and missing Team profiles, multilingual original messages, and messages shaped like prompt instructions. | The final ephemeral followup attaches both UTF-8 `shift-draft-notes.txt` and `shift-draft-llm-prompt.txt`. The prompt contains every participant and complete raw message, administrator text, exact availability, Team values/status, Runner, threshold, baseline Draft, and baseline workload/switch metrics from the same in-memory generation snapshot. The Runner Entry keeps the exact canonical Runner name and raw note, is marked `is_fixed_runner`, and cannot be placed in any supporter column. Embedded data cannot change the fixed scheduling or response instructions. No Summary or Draft read-back occurs. |  |  |
| Generate Draft LLM fallback and empty Entry | Generate once with Team Source unavailable and once with no Shift participants. | The unavailable-source prompt does not guess ISV, Power, or role eligibility and requires Encore to stay blank. The empty prompt requests the exact number of five-blank-cell rows and an explicit shortage summary. Draft generation and the existing Notes behavior otherwise remain unchanged. |  |  |
| Generate Draft LLM audit and paste | Submit the generated prompt to an external LLM with conflicting administrator and participant needs, insufficient staffing, long runs, and frequent role changes. Include one case where the highest-ISV Encore or Honso choice conflicts with continuity or workload, and one where lower Main ISV standby conflicts with continuity. Review the result, then copy only the data between the TSV markers and paste it at `C2`. | The Traditional Chinese summary reports hard checks, each requirement, ignored or unsatisfied needs with reasons, shortages, total/longest/Encore hours, switches, rest concerns, and principal baseline changes. Higher Encore/Honso ISV and lower standby Main ISV are applied only as soft tie-breaks when other considerations are comparable; the result may choose the opposite ISV direction for participant needs, continuity, workload, rest, or switching efficiency. The marked block has exactly five tab-separated columns and one row per visible JST hour, with five blank cells for gaps and only exact canonical names or blanks. Pasting changes only `C:G`; JST and Runner remain unchanged and dynamic Notes recalculate. |  |  |
