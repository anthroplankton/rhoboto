...
| Spreadsheet-scoped batch reads | With request logging enabled, submit Team data, submit a Shift with Team Source, and generate Draft. Repeat with all Team worksheets and Team Summary in the same spreadsheet as Shift Entry/Draft and in a separate spreadsheet. | Each locked read-plan-write phase sends one `values.batchGet` per spreadsheet. Draft generation reads Entry, Draft, every configured Team worksheet, and Summary in one request when they share a spreadsheet; an external Team spreadsheet adds exactly one request for all of its Team worksheets and Summary. Final Schedule content is never included, and Summary is not read back after refresh. |  |  |
