...
| Final Schedule per-command ownership | Put sentinels, formatting, validation, formulas, and notes throughout Final Schedule; run Shift registration, settings repair, Draft generation, then `generate_final`. | Shift registration, settings repair, and Draft generation make no Final value/format request. `generate_final` reads no Final values and writes only the confirmed current main rectangle plus an optional date cell; it formats only role-cell backgrounds/foregrounds, never Runner, and leaves all properties outside that narrow footprint unchanged. |  |  |
...
| Generate Final overwrite confirmation | Configure recruitment ranges and a saved Final anchor, then invoke `/shift_register generate_final` with and without optional date inputs. Verify the `event_day_format` option shows its default, rejects more than 512 characters, and preserves literal full-width text while accepting NFKC-equivalent token identifiers. Inspect the prompt without pressing a button. | The prompt links Shift Draft and Final Schedule once, shows `B2:G{last row}` and the exact Final target rectangle, previews the formatted event date when valid, discloses omitted/invalid date writes, warns to back up and that a shorter run retains stale tail cells, and notes any changed anchor will be saved only after Sheet success. No metadata, worksheet repair, or value request occurs before confirmation. |  |  |
| Generate Final Draft contract repair | Alter the Draft header, one expected JST axis cell, or one role value so it violates the Draft contract, then confirm `generate_final`. | No Final mutation occurs. The `⚠️📏` response identifies the location when available and shows bounded, mention-safe expected and detected values without exposing internal labels such as `header`, `axis`, or `role_value`. |  |  |
