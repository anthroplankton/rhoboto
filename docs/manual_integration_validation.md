...
| Spreadsheet-scoped batch reads | With request logging enabled, submit Team data, submit a Shift with Team Source, and generate Draft. Repeat with Team Summary in the same spreadsheet as Shift Entry/Draft and in a separate spreadsheet. | Each locked read-plan-write phase sends one `values.batchGet` per spreadsheet. Team worksheets and Summary share one request; same-spreadsheet Entry/Draft/Summary share one request; an external Team Summary adds exactly one request for that spreadsheet. Final Schedule content is never included. |  |  |
