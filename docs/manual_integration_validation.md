...
| Generate Draft overwrite confirmation | Configure recruitment ranges `4-12, 20-28`, invoke generate, and inspect the prompt without pressing a button. | `Shift Draft` links directly to the configured Draft worksheet. The ephemeral prompt lists only new destinations: Draft `A1:G31`, Notes `A27`, candidate `I1` and threshold `I26:K26`, lookup `J28:L30`, plus conditional Team data from `J31`. No Google Sheets API request or worksheet repair occurs before confirmation. |  |  |
