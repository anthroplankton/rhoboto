...
| Finalization after soft disable or hard clear | Soft-disable Shift Register, then run `generate_draft`, `update_schedule_from_draft`, and `assign_schedule_role`; hard-clear settings and repeat. | Soft-disabled configured settings still allow all three finalization commands. After hard clear, each command reports missing Shift Register configuration without reading or writing Sheets. |  |  |
...
| Assign Schedule Role failures and concurrency | Remove effective `Manage Roles`, inject individual add/remove failures, then issue same-role and different-role requests concurrently. | Missing permission and individual failures appear in Screen A while independent operations remain reported. Requests for the same guild/role serialize and avoid redundant calls; different roles can proceed concurrently, and Sheet locks are released before the role lock. |  |  |
