...
| Settings embed with one Team source | Configure exactly one Team Register in the same guild, then run `/shift_register settings`. | Embed shows worksheet titles/IDs, Sheet link, Team Source channel and Team Register Google Sheet link, final schedule anchor cell, Shift Timeline, and Recruitment Time Range. The Team Source does not repeat its landing worksheet title or ID. |  |  |
...
| Missing Team source | Remove all Team Register configuration from the development guild, then reopen Shift settings. | Team Source says no configured Team Register exists. Shift registration still updates availability. |  |  |
| Ambiguous Team source | Configure Team Register in two channels in the same guild, then reopen Shift settings. | Team Source reports multiple configured Team Registers and that source selection is not yet supported. Shift registration still updates availability and stale `C` anchors are cleared. |  |  |
