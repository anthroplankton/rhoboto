...
- 對 `アンコ` 與 `本走`，若無更高優先限制，應避免只排一小時就換人，並優先
  維持合理的連續班。兩小時只是連續性的參考，不是上限、固定分組或必須切開的區塊；
  需求、負荷與休息允許時，同一人可以連續三小時以上。
- `待機` 是臨時備援，不要求連續兩小時，也不必與 `アンコ`／`本走` 同時上下班。
  此項豁免不改變待機的逐時 Main ISV 選擇、參加者需求、總負荷與休息規則。
- 避免頻繁換人或變更支援角色，因為會拖慢效率；避免總時數或連續時數過長，
  長班後安排休息。
...
- 原則上優先選擇支援角色對應 ISV 較高者；
  ...
  維持 `アンコ` 或 `本走` 的合理連續班（可超過兩小時）、
  減少換人或變更支援角色、促成 `アンコ` 與 `本走` 人員同步交接，
  或改善負荷與休息時，才可採用。
  稽核摘要必須說明哪些時段／支援角色選用了較低 ISV 人員，以及換得哪些改善。
- 若確實需要更換 `アンコ` 或 `本走`，應將兩種支援角色的人員同步交接納入效率權衡，
  不必等所有品質項目完全相同才考慮；此同步交接偏好不包含 `待機`，也不得為此
  違反更高優先規則，或不必要地中斷仍符合需求、負荷與休息的 `アンコ`／`本走`
  連續班。
...
- 是否把兩小時誤當成上限或固定分組，或錯誤要求待機連續兩小時或與
  `アンコ`／`本走` 人員同步上下班；
- 本走欄位是否已改善視覺連續性；
